<?php
	...
	$num_users = mysqli_query($dbc, "SELECT * FROM teamcode"); 
		...
		$fixed_result = mysqli_query($dbc, "SELECT MAX(fixed_group) FROM teamcode"); 
...
$user_id = $num_users->fetch_assoc();  //fetch row.	
	...
	while ($count <= $tag_count)
	{
		...
		$group_result = mysqli_query($dbc, "SELECT users_id FROM teamcode WHERE random_group = $rndNum"); 
